...
export function NightlyReviewDialog({ open, onOpenChange }: NightlyReviewDialogProps) {
    const { user } = useUser();
    const firestore = useFirestore();
    const { toast } = useToast();
    const [step, setStep] = useState<Step>('win');
    const [data, setData] = useState<ReviewData>({ win: "", learning: "", gratitude: "" });
            ...
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    <div className="flex items-center gap-2 text-indigo-500 mb-2">
                        ...
                        <span className="text-sm font-medium uppercase tracking-wider">Nightly Review</span>
                    </div>
                ...
                <div className="py-4 min-h-[200px]">
                    {step === 'win' && (
                        <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                            <Label className="text-lg font-medium">What was your biggest win today?</Label>
                            <p className="text-sm text-muted-foreground">Even a small step forward counts.</p>
                            <Input
                    ...
                    {step === 'learning' && (
                        <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                            ...
                            <p className="text-sm text-muted-foreground">About yourself, your work, or the world.</p>
                            <Input
                    ...
                    {step === 'gratitude' && (
                        <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                            <Label className="text-lg font-medium">What are you grateful for?</Label>
                            <p className="text-sm text-muted-foreground">Focus on one specific thing.</p>
                            <Input
                        ...
                        <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                            ...
                            <div className="space-y-3 text-sm">
                                <div className="bg-muted/30 p-3 rounded">
                                    <span className="block text-xs font-bold uppercase text-muted-foreground mb-1">Win</span>
                                    {data.win}
                                </div>
                                <div className="bg-muted/30 p-3 rounded">
                                    <span className="block text-xs font-bold uppercase text-muted-foreground mb-1">Learning</span>
                                    {data.learning}
                                </div>
                                <div className="bg-muted/30 p-3 rounded">
                                    <span className="block text-xs font-bold uppercase text-muted-foreground mb-1">Gratitude</span>
                                    {data.gratitude}
                                </div>
                            </div>
                            ...
                            <div className="bg-indigo-50 dark:bg-indigo-950/30 p-3 rounded-md text-xs text-indigo-800 dark:text-indigo-300 flex gap-2">
                                ...
                                <p>This data will help calibrate your future plans and Morning Briefings.</p>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <DialogFooter className="flex justify-between sm:justify-between w-full">
                    ...
                    {step === 'preview' ? (
                        <Button onClick={handleSubmit} disabled={isSubmitting} className="bg-indigo-600 hover:bg-indigo-700">
                            {isSubmitting ? "Saving..." : "Complete Review"} <Check className="w-4 h-4 ml-2" />
                        </Button>
                    ) : (
