...
export function DecisionDialog({ open, onOpenChange, activeVisions }: DecisionDialogProps) {
    const { user } = useUser();
    const firestore = useFirestore();
    const { toast } = useToast();
    const [decision, setDecision] = useState("");
    const [analysis, setAnalysis] = useState<AnalyzeDecisionAlignmentOutput | null>(null);
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            await addDoc(collection(firestore, 'users', user.uid, 'decision_journal'), {
                decision,
                visions: activeVisions,
                analysis,
    ...
    const getScoreColor = (score: number) => {
        if (score >= 8) return "text-green-600 bg-green-100 dark:bg-green-900/30 dark:text-green-400";
        if (score >= 5) return "text-yellow-600 bg-yellow-100 dark:bg-yellow-900/30 dark:text-yellow-400";
            ...
            <DialogContent className="sm:max-w-lg">
                <DialogHeader>
                    <div className="flex items-center gap-2 text-indigo-500 mb-2">
                        ...
                        <span className="text-sm font-medium uppercase tracking-wider">Decision Alignment</span>
                    </div>
                    ...
                    {!analysis ? (
                        <div className="space-y-4">
                            ...
                            <div className="text-xs text-muted-foreground">
                                Analyzing against <strong>{activeVisions.length} active visions</strong>.
                            </div>
                        </div>
                    ) : (
                        <div className="space-y-4 animate-in fade-in zoom-in-95 duration-300">
                            <div className="flex items-center justify-between">
                                ...
                                <div className={cn("text-2xl font-bold px-3 py-1 rounded-md", getScoreColor(analysis.score))}>
                                    {analysis.score}/10
                                </div>
                            </div>
                            ...
                            <div className="bg-muted/30 p-4 rounded-lg space-y-3">
                                ...
                                <div className="space-y-1">
                                    <span className="text-xs font-bold uppercase text-muted-foreground">Analysis</span>
                ...
                <DialogFooter>
                    {!analysis ? (
                        <Button onClick={handleAnalyze} disabled={!decision.trim() || isAnalyzing} className="w-full sm:w-auto">
                            {isAnalyzing ? (
                                <>
