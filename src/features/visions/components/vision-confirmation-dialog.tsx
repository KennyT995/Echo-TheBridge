    ...
    return (
        <div className="animate-in fade-in zoom-in-95 duration-200">
            <div className="mb-6 text-center space-y-2">
                <h3 className="text-xl font-bold">I noticed multiple goals in your vision.</h3>
                <p className="text-muted-foreground">{analysis.reasoning}</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
                <Card className="border-2 border-primary/20 hover:border-primary/50 transition-colors cursor-pointer group" onClick={onConfirmSeparate}>
                    <CardContent className="pt-6 flex flex-col h-full">
                        ...
                        <div className="space-y-2 mt-auto">
                            {analysis.proposedVisions.map((vision, idx) => (
                                <div key={idx} className="bg-muted p-2 rounded text-sm font-medium">
                                    {vision.title}
                                </div>
                            ))}
                        </div>
                        <Button className="w-full mt-4" variant="outline" disabled={isCreating}>
                            Separate Them
                        </Button>
                    </CardContent>
                </Card>

                <Card className="border-2 border-primary/20 hover:border-primary/50 transition-colors cursor-pointer group" onClick={onConfirmUnified}>
                    <CardContent className="pt-6 flex flex-col h-full">
                        <div className="mb-4 p-3 bg-primary/10 w-fit rounded-full group-hover:bg-primary/20 transition-colors">
